...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        ...
        let mut reason: &str = "";
        let allowed:[&str; 3] = ["To-do", "In Progress", "Done"];

        if !allowed.contains(&status.as_str()) {
            is_valid = false;
            reason = "Only `To-Do`, `In Progress`, and `Done` statuses are allowed"
        };
        ...
        if title.len() > 50 {
            is_valid = false;
            reason = "Title cannot be longer than 50 bytes"
        }

        if description.len() > 500 {
            is_valid = false;
            reason = "Description cannot be longer than 500 bytes"
        }
